...
const ShoppingCard = () => {
  ...
  const [counterInputVal, setCounterInputVal] = useState(0);

  useEffect(() => {
    setCounterInputVal(items);
  }, [items]);
    ...
    <section className="counter-wrapper flex flex-col items-center w-2/3 gap-8 p-8 bg-white border-4 shadow-lg border-primary-500">
      ...
      <div className="counter-form">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            setItems(+counterInputVal);
            setCounterInputVal(0);
          }}
        >
          <input
            type="number"
            value={counterInputVal}
            onChange={(e) => setCounterInputVal(e.target.valueAsNumber)}
